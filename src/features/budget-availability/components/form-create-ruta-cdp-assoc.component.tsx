...
interface FormularioProps {
  ...
  datasFounds: any;
  countAssoc?: number;
  setFormDataArray?: any;
  handleFormSubmitSaveData?: any;
}

const FormCreateRutaCDPAssocComponent: React.FC<FormularioProps> = ({
  countAssoc,
  datasFounds,
  formSubmitted,
  isRequired = false,
  ...
  let renderedFormNumber =
    posicionCdp && posicionCdp !== 0
      ? Number(formNumber) > posicionCdp
        ? Number(formNumber)
        : posicionCdp
      : Number(formNumber);
  ...
  const fetchData = async () => {
    if (!datasFounds) {
        ...
        try {
          ...
          if (typeof response === "object") {
            ...
            setMessage({
              ...
              description:
                "No encontramos una ruta presupuestal con los datos que proporcionaste, intentalo de otra vez con nuevos datos",
              show: true,
              OkTitle: "cerrar",
              onOk: () => {
                setMessage({});
      ...
      if (
        datasFounds.valorInicial !== "0" &&
        tryJsonInfo === datasFounds.idRpp
      ) {
        console.log("TrySet 2");
...
/*   const loadInfo = () => {
    let arrInfo = [];
    const existencia = formDataCdpRoute.some((obj) => obj.id === renderedFormNumber);
    ...
    arrInfo.push(newObj);
    const exists = formDataCdpRoute.some((obj) => obj.id === renderedFormNumber);
    const existingIndex = formDataCdpRoute.findIndex((obj) => obj.id === renderedFormNumber);
    ...
    if (arrInfo && arrInfo.length > 0) {
      if (!exists) {
        ...
        const updatedArrInfo = [...formDataCdpRoute];
        updatedArrInfo[existingIndex] = newObj;
        setFormDataCdpRoute(updatedArrInfo);
  ...
  const loadInfo = () => {
    // Nuevo objeto con la información del formulario actual
    const newFormData = {
      proyecto,
      nombreProyecto,
      fondo,
    ...
    // Actualizamos el estado local del formulario hijo
    ...
    setAmountInfo((prevAmountInfo) => ({
      ...
      handleFormSubmitSaveData(newFormData);
  };
  useEffect(() => {
    loadInfo();
  }, [
  ...
  const handleNombreProyectoChange = (
  ...
  const handleAreaFuncionalChange = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    setAreaFuncional(event.target.value);
  };

  const handleCentroGestorChange = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
      ...
      <div className="formulario">
        <div>
          <h2 className="h3-style" style={{ flex: 1 }}>
            {countAssoc && countAssoc > 0 ? countAssoc : renderedFormNumber + 1}
            . Ruta
          </h2>
          <div
            style={{
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} sm={12} md={4}>
              <div className={`col-4`}>
                ...
                <SelectSearch
                  defaultValue={parseInt(proyecto)}
                  style={{
                    border:
                      formSubmitted && !proyecto
                        ? "1px solid red"
                        : "1px solid #ccc",
                  }}
            ...
            <Grid item xs={12} sm={12} md={8}>
              <div className={`col-8`}>
                <label>
                  Nombre proyecto <span>*</span>
                </label>
                <input
                  type="text"
                  style={{
                    border:
                      formSubmitted && !nombreProyecto
                        ? "1px solid red"
                        : "none",
                  }}
                  className={`estilo-input bgSecondary ${validateField(
                    nombreProyecto
                  )}`}
            ...
            <Grid item xs={12} sm={12} md={4}>
              <div className={`col-4`}>
                ...
                <SelectSearch
                  defaultValue={pospreNewV}
                  style={{
                    border:
                      formSubmitted && !pospreNewV
                        ? "1px solid red"
                        : "1px solid #ccc",
                  }}
                  options={convertedOptionsPosPre}
                  setter={setPospreNewV}
                />
                {formSubmitted && !pospreNewV && (
                  <p className="aviso-campo" style={{ color: "red" }}>
                    Este campo es obligatorio
            ...
            <Grid item xs={12} sm={12} md={4}>
              <div className={`col-4`}>
                <label>
                  Área funcional <span>*</span>
                </label>
                <input
                  style={{
                    border:
                      formSubmitted && !areaFuncional
                        ? "1px solid red"
                        : "none",
                  }}
                  type="text"
                  ...
                  value={areaFuncional}
                  onChange={handleAreaFuncionalChange}
                />
                {formSubmitted && !areaFuncional && (
                  <p className="aviso-campo" style={{ color: "red" }}>
                    Este campo es obligatorio
            ...
            <Grid item xs={12} sm={12} md={4}>
              <div className={`col-4`}>
                ...
                <input
                  type="text"
                  readOnly
                  className={`estilo-input bgSecondary ${validateField(div)}`}
                  value={div}
            ...
            <Grid item xs={12} sm={12} md={4}>
              <div className={`col-4`}>
                ...
                <input
                  type="text"
                  readOnly
                  className={`estilo-input bgSecondary`}
                  value={
                    countAssoc && countAssoc > 0
                      ? countAssoc
                      : renderedFormNumber + 1
                  }
...
export default FormCreateRutaCDPAssocComponent;
