...
const CdpAmountAssoc = () => {
  ...
  const handleAgregarFormulario = () => {
    ...
    const nextId2 =
      countNewFormsSt.reduce((max, obj) => (obj.id > max ? obj.id : max), 0) +
      1;
  ...
  const handleEliminar = (formNumber) => {
    ...
    setCountNewFormsSt((prevFormularios) =>
      prevFormularios.filter(
        (_, index) => indexOfFirstForm + index !== formNumber
      )
    );
    ...
    try {
      ...
      if (invalidBalances.length !== 0) {
        setMessage({
          ...
          description:
            "Recuerda que el valor inicial no puede ser mayor o igual al balance disponible de la ruta presupuestal",
          show: true,
          OkTitle: "Aceptar",
          onOk: () => {
            setMessage({});
      ...
      if (invalidBalances.length === 0) {
        const updatedIcdArr = icdArrWithBalanceCheck.map((item) => {
          const valuesAreValid = Object.values(item).every(
        ...
        setMessage({
          ...
          onOk: async () => {
            try {
              const response = await cdpService.associateCdpAmounts(
                nuevoObjeto
              );
  ...
  useEffect(() => {
    getCdpById(lastSegment).then((res) => {
      ...
      const lastAmount = amounts.length + 1;
